...
Android **P0 真联调 Phase 1** 完成：Auth 接真 API + 三页布局重设计 + **真机 USB（adb reverse）** 联调路径就绪；词书/分组仍 Mock。

| 区域 | 状态 |
|------|------|
| 五个 Tab | **全部 Mock UI 可点通**（设置 / 词书 / 分组 / 统计 / 今日） |
| 子页 | 学习 / 分组详情 / 默写测验 / 卡拍 / **CustomGroup** / **找回密码** |
| 词书 | **导入 preview→confirm**（本地解析 TXT/CSV/JSON）；**手动添加分组** chips 多选 |
| 认证 | **真 API** register/login/logout；EncryptedSharedPreferences Token；401 refresh；登录/注册/找回密码 **单卡片布局**；模拟验证码 |
| 真机联调 | debug `API_BASE_URL`=`127.0.0.1:8080` + `scripts/adb-reverse.ps1`；`wordflip-server/scripts/start-dev.ps1` |
| 设置 | DataStore 持久化自动发音（默认开）+ 主题；退出登录清 Token 回 Auth |
...
**已打通导航：** 今日 ↔ 学习 / 测验；词书 → CustomGroup / 导入；词书保存；分组详情 → 学习 / 测验；分组卡 → 卡拍 / 污渍模式；登录 ↔ 注册 ↔ **找回密码**。

**建议下一波：** P0-T04 真机走通注册→登录→今日；P0-B31~35 Groups 读 API；P0-A 词书/分组接真 API。
...
- [ ] P0-T04 Android 真机/模拟器走通 P0 全流程 — Auth 真机 USB 路径已就绪，待完整 P0 走查
