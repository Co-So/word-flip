...
> 版本：v6.6（v5 原型 + 后端架构定稿补充；词库结构化契约）  
> 整理日期：2026-07-10  
> 文档性质：用户可见行为与业务逻辑规格，不含技术实现细节

本文档规定 WordFlip 从用户视角**必须做什么、在什么条件下做什么、如何反馈**。每条需求以 `REQ-<模块>-<序号>` 编号，便于追溯与拆分用户故事。

> **词库结构化（契约已定）：** 一词多义 Headword 1—n Sense；展示/测验默认 **primary** sense。表落地与读路径见 [plans/lexicon-restructure.md](./plans/lexicon-restructure.md) Phase C/D；在此之前运行时仍可能用扁平列 + 清洗治标，但产品行为以本节 REQ-LEX / 相关 REQ 为准。

**需求来源优先级**：

1. 本文档 **v6 定稿**（掌握度三态、测验为准、分组增量追加、账号登录、词条义项）  
...
| wordKey | `en.trim().toLowerCase()`；学习进度 / 图片 / 污渍 / 分组均绑 `(user_id, wordKey)` |
...
- **REQ-LEX-2：** 卡片背面、默写题干、选择题正确项与干扰项、答错后展示的「正确答案」均使用该词的 **primary** 义项（`cn` + `pos` + 词头 `ph`/`en`）。
