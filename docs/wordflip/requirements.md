...
> **待修订（词库结构化）：** 一词多义 1:n、字段职责（en/cn/pos/ph/examples）、测验只用 primary sense — 见实施计划 [plans/lexicon-restructure.md](./plans/lexicon-restructure.md)。在计划 Phase A 完成前，现行扁平 `cn` 字符串模型仍为运行时行为；冲突时以计划中「已拍板决策」为准推进改造。
