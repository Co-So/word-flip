...
| 掌握度 | 单词学习状态：**未学习**、**模糊**、**不认识**（三档）；**以默写测验结果为最终依据** |
| 已掌握（统计） | 非独立状态；指测验通过后复习间隔已拉长的单词（见 REQ-EBBING） |
...
- **REQ-TODAY-12：** 「已掌握单词数」为统计指标：复习阶段已达上限或下次复习日期在 30 天以后的单词数，**不是**独立的掌握度第四档。
...
- **REQ-GROUP-2：** 每个分组卡片应展示：组名、状态标签（已完成 / 学习中 / 未开始）、四维统计（**未学习 / 模糊 / 不认识 / 总词**）、进度条（非「不认识」且已测验通过占比，或按业务统计口径展示）。
...
- **REQ-GDETAIL-2：** 列表模式下，页面应逐条展示组内单词：英文、中文释义、词性，以及当前掌握度标签（**未学习 / 模糊 / 不认识**）。
- **REQ-GDETAIL-3：** 掌握度 **以默写测验结果为最终依据**；列表模式 **不提供** 手动修改掌握度的按钮（v5 的「记得/模糊」按钮在正式版中移除）。
- **REQ-GDETAIL-4：** 用户可通过列表进入学习或测验以更新状态；最近一次测验结果决定展示标签。
...
- **REQ-QUIZ-1：** 默写测验模式为：展示中文释义（及词性、音标），用户输入对应英文单词。
- **REQ-QUIZ-2：** 每次进入测验页，系统须重新初始化：从服务端抽取题目（默认池见 REQ-TODAY-11），重置题号、得分与错题列表。
- **REQ-QUIZ-3：** 每题界面应包含：顶部进度条、题号与总题数、当前得分、中文卡片、输入框、「确认」按钮。
- **REQ-QUIZ-4：** 进入每题后，输入框应自动获得焦点。

### 12.2 判题、掌握度与反馈

- **REQ-QUIZ-5：** 当用户点击确认或按回车提交时，系统应将输入与标准答案比较，**忽略大小写**；提交后输入框应禁用防重复提交。
- **REQ-QUIZ-6：** **掌握度以测验为最终依据**；服务端判题后更新状态与复习计划：
  - **答对** → 掌握度设为 **未学习**（表示当前测验通过），并按 SRS 间隔延长 `next_review_at`；UI 显示「✓ 正确！」、得分 +1。
  - **答错（首次或单次）** → 掌握度设为 **模糊**，`next_review_at` 设为尽快复习（如 +1 天）；UI 显示「✗ 正确拼写：{英文}」。
  - **答错且同一单词连续 2 次测验错误** → 掌握度设为 **不认识**，优先进入待复习队列。
- **REQ-QUIZ-7：** 判题展示约 1.4 秒后，系统应自动进入下一题。
- **REQ-QUIZ-8：** 测验结果应持久化至服务端（会话、每题作答、掌握度与复习计划变更）。
...
| REQ-DATA-8 | 掌握度（未学习/模糊/不认识） | **默写测验判题后** | 分组列表、详情、今日任务 |
...
- **REQ-EBBING-2：** **测验答对** → 掌握度 **未学习**，复习阶段 +1，按序列设置 `next_review_at`。
- **REQ-EBBING-3：** **测验答错（单次）** → 掌握度 **模糊**，阶段回退或保持低位，`next_review_at` 设为 +1 天（尽快复习）。
- **REQ-EBBING-4：** **同一单词连续 2 次测验答错** → 掌握度 **不认识**，`next_review_at` 设为当日或 +1 天，优先进入待复习。
- **REQ-EBBING-5：** 今日待复习 = `next_review_at <= 当日` 的单词，排序：不认识 > 模糊 > 未学习（已到期）。
- **REQ-EBBING-6：** 卡片翻转、浏览学习 **不直接** 改变掌握度与复习计划；以测验为准。
- **REQ-EBBING-7：** 「已掌握」为统计口径（如阶段达上限且下次复习 > 30 天），**不是**用户可选的第四档掌握度。
...
| 掌握度三档 | 记得/模糊/不认识 | 记得/模糊 | **未学习/模糊/不认识，测验为准** |
