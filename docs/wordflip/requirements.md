...
> 版本：v7.0（词书专属学习卡 + FSRS）
> 整理日期：2026-07-16
> 文档性质：用户可见行为与业务逻辑规格，不含技术实现细节

本文档规定 WordFlip 从用户视角**必须做什么、在什么条件下做什么、如何反馈**。每条需求以 `REQ-<模块>-<序号>` 编号，便于追溯与拆分用户故事。

> **内容体系（契约已定）：** 用户学习的是当前词书的已发布学习卡；同一词形在不同词书中可有不同考义。ECDICT、WordNet 等只提供可追溯的详情资料，不再作为可切换的全局展示义。

**需求来源优先级**：

1. 本文档 **v7 定稿**（单主词书、词书专属学习卡、FSRS、双层记忆）
2. `prototypes/wordflip-v5.html` 已实现交互（UI/动效参考）  
3. `docs/prd/WordFlip-PRD.md` 规划描述  

v5 原型与 v6 冲突时，以 **v6 业务定稿** 为准（尤其掌握度、分组生成、登录、释义真相）；v5 独有 UI 交互仍以原型为准。
...
- **REQ-GLOBAL-3：** 用户可保留多本词书和历史学习计划，但任一时刻只有一本当前主词书；学习、分组、今日任务和测验仅使用当前计划中的学习卡。
...
| 学习卡 | 某一本词书中某个词形的专属考义内容；以 `cardId` 为学习单位 |
| 学习计划 | 用户针对一本词书的学习配置和历史；`activePlanId` 唯一指向当前计划 |
| 卡片记忆 | `card_skill_memory`；当前词书考义的权威 FSRS 进度，按 skill 独立 |
| 词形熟悉度 | `lexeme_skill_memory`；跨词书诊断参考，不能直接把新词书学习卡标记为掌握 |
| 热力档 heatLevel | 由 S 映射的 0–4 档（新词→很熟），须 **颜色 + 文案**；展示档可由 `heatDisplayMode` 聚合 |
| skill（题型技能） | `dictation`（默写）与 `choice`（选择）双轨；各有独立 FSRS 状态 |
| 已掌握（统计） | 当前卡片在对应 skill 中处于 review 且建议间隔达到产品统计阈值；不是用户可选状态 |
| 污渍 | 卡片正面英文面上的视觉记忆锚点标记 |
| 卡拍 | 为卡片背面拍摄或导入图片，形成图像化记忆 |
| 艾宾浩斯复习 | 按间隔重复安排到期复习的学习机制 |
| wordKey | `en.trim().toLowerCase()`；仅作为规范词形查询键，不再作为学习进度主键 |
| Headword | 词头：英文 `en` + 音标；对应一个 `wordKey` |
| Sense（义项） | 一条「词性 + 中文释义」；一词可有多条；例句挂在义项下 |
| primary sense | 该词默认展示与测验所用的主义项；每个合格词恰好一条 |
| quality | 义项质量：`ok` / `uncertain` / `reject`；仅 `ok` 的 primary 可入测验池 |
...
字段职责必须分清，禁止把词性、多义、英文短语塞进同一条「中文」字符串当真相：

| 字段 | 职责 | 基数 |
|------|------|------|
| `en` / `wordKey` | 英文词头；`wordKey = en.trim().toLowerCase()` | 1 |
| `ph`（及可选美音） | 音标，属词头 | 1（可空） |
| `pos` | 词性，属**义项**；不得写入 `cn` | 每义项 0..1 |
| `cn` | **纯中文释义**（须含汉字）；不含词性尾巴 | 每义项 1 |
| `examples` | 例句（英 + 中），挂在义项下 | 每义项 0..n |

- **REQ-LEX-1：** 一个英文词头与中文释义为 **1:n**（多义）；每个义项可有 0..n 条例句。
- **REQ-LEX-2：** 卡片背面、默写题干、选择题正确项与答错反馈均使用当前词书学习卡的主义项，不得被详情词典覆盖。
- **REQ-LEX-3：** 长按详情应展示该词**全部**义项及各义项例句；不得把多义拼成一条不可拆的 `cn`。
- **REQ-LEX-4：** 未发布、待补充或没有合格主义项的学习卡不得进入学习与测验池。
- **REQ-LEX-5：** 词书导入与内置词灌库须经结构化清洗（规则优先，不确定项可离线 LLM）；入库义项须满足：`cn` 含汉字、词性只在 `pos`、每词至多一个 primary 且 primary 须为 `ok`（否则该词不可测）。
- **REQ-LEX-6：** 权威复习进度按 `(userId, cardId, skill)` 记录；跨词书词形熟悉度按 `(userId, lexemeId, skill)` 记录。
- **REQ-LEX-7：** 每个词书条目只能有一个当前发布版学习卡；每张发布卡至少有一个合格主义项，且义项能追溯到词书原文、用户输入或具体来源修订。
- **REQ-LEX-8：** ECDICT 原始字段与原始释义必须无损保存；结构化义项是派生内容，不得破坏性覆盖原始记录。
- **REQ-LEX-9：** 取消全局词典切换。详情抽屉按来源分区展示 ECDICT、WordNet 等资料，资料来源不得改变当前词书考义。
- **REQ-LEX-10：** 用户导入的中文释义只属于该用户的词书学习卡，不进入公共内容库；只有英文时可生成候选，无可靠候选则标记待补充且不进入测验池。
...
- **REQ-NAV-7：** 当用户进入词书主页面时，系统应刷新词书列表、当前计划与各书学习进度，反映最近一次切换或导入结果。
...
- **REQ-TODAY-1：** 今日页主内容区应在常见手机屏高下尽量展示核心信息；**「最近学习」分组列表**作为主入口置于任务区上方（不再使用底部固定「开始学习」大按钮）。
...
- **REQ-TODAY-7：** （已调整）原底部「开始学习」大按钮移除；进入推荐分组学习改由「最近学习」主点击或任务行完成。`recommendedStudy` 仍可由服务端返回供任务副标题等使用。
- **REQ-TODAY-8：** 当用户点击顶部提醒图标时，系统应以 Toast 等方式反馈提醒相关状态（具体提醒能力见设置页规划项）。
- **REQ-TODAY-13：** 今日页应在任务区上方展示「最近学习」分组列表（最多 3 条：组名 + 最近学习时间）；**主点击进入该组卡片学习**，次要操作可发起测验 `source=recent`。卡片学习与测验完成均应更新该列表。
...
- **REQ-BOOK-2：** 每本词书展示名称、已发布学习卡数、审核状态与该用户的学习进度。
- **REQ-BOOK-3：** 首次使用必须选择一本主词书并创建学习计划，之后才能进入今日、分组、学习与测验。
- **REQ-BOOK-4：** 切换词书等同切换学习计划；旧计划与进度保留，但任一时刻 `activePlanId` 只能指向一个计划。
...
- **REQ-BOOK-6：** 当用户触发导入并选择文件后，系统应解析文件内容；支持格式包括：
  ...
  - 解析后须按 REQ-LEX-5 **结构化清洗**（拆义项、剥词性尾巴、拒绝无汉字/英文充中文等）；不合格词可入库浏览但不得进入测验池。
- **REQ-BOOK-7：** 解析成功后，系统应弹出确认面板，包含：可编辑的词书名称、前若干条单词预览、总词数提示。
- **REQ-BOOK-8：** 用户确认导入后，系统创建仅该用户可见的词书及学习卡候选；有可靠考义的卡可发布，无可靠考义的条目标记待补充。
- **REQ-BOOK-9：** 当用户点击确认面板「取消」或点击遮罩关闭时，系统应放弃本次导入，不写入数据。
- **REQ-BOOK-10：** 当文件无法解析、无有效单词或存储失败时，系统应显示明确错误 Toast，不创建词书记录。
- **REQ-BOOK-11：** 导入词书可删除，但存在历史学习计划或复习事件时只能归档，不得破坏审计记录。
...
- **REQ-BOOK-13：** 分组大小作用于当前学习计划；修改时不得删除卡片记忆或复习事件。
- **REQ-BOOK-14：** 汇总词数等于当前词书的已发布学习卡数；声明词数仅作展示参考。
- **REQ-BOOK-22：** 用户应可从三种自动分组策略中选择其一：**词书顺序**（`book_order`）、**词频**（`frequency`）、**随机**（`random`）；默认 **词书顺序**。
- **REQ-BOOK-23：** 词书顺序策略按 `book_items.sort_order` 和当前发布学习卡排序，不跨书合并去重。
- **REQ-BOOK-24：** 随机策略按 `(userId, planId)` 稳定随机；词频策略按当前词书条目频率元数据排序。
- **REQ-BOOK-27：** 词书页提供「开始学习」或「切换到此计划」；切换后重新计算当前计划分组与今日任务。
- **REQ-BOOK-25：** 增量追加分组时，若最后一个 `source=auto` 分组未满 `groupSize`，应 **先补齐该组** 再按策略切分剩余 delta 创建新组；**不得**拆分或合并已有完整分组。
- **REQ-BOOK-26：** 重新分组只重建当前计划的自动分组与成员关系；`card_skill_memory`、`lexeme_skill_memory`、复习事件、图片和污渍不得删除。

**规划项（未实现）：** 用户自定义输入任意分组大小（PRD 要求，v5 仅四档预设）。

### 5.4 保存、分组增量追加与自定义分组入口

- **REQ-BOOK-15：** 创建或切换学习计划时应原子更新 `activePlanId`，确保唯一当前主词书。
- **REQ-BOOK-17：** 自动分组只从当前计划词书的已发布学习卡中追加尚未入组的卡片。
- **REQ-BOOK-18：** 分组变更不得修改双层记忆或复习事件；新卡默认是 FSRS `new` 状态。
- **REQ-BOOK-19：** 内容版本新增学习卡后，可幂等追加到当前计划；已存在 `cardId` 不得重复入组。
- **REQ-BOOK-20：** 切换词书后旧计划的分组和进度保留，但不参与新计划的今日任务。
- **REQ-BOOK-21：** 每张学习卡在同一学习计划下只属于一个学习分组。
- **REQ-BOOK-16：** 手动添加分组的候选池为当前计划中已发布且尚未入组的学习卡。

```mermaid
flowchart TD
  A[保存当前计划设置] --> B[读取当前词书已发布学习卡]
  B --> C[减去已在组内的 cardId]
  C --> D{delta 非空?}
  D -->|否| E[仅更新计划设置]
...
- **REQ-CG-3：** 当候选池为空时，系统应提示「当前词书没有未入组的已发布学习卡」，且不展示 chips。
...
- **REQ-STUDY-4：** 卡片正面应展示：英文单词、**primary 词性**、可选污渍（未隐藏时）、底部「翻转」提示。
- **REQ-STUDY-5：** 卡片背面分两种状态（释义均为 **primary** sense，见 REQ-LEX-2）：
  - **无用户图片**：展示 primary 中文释义、音标、「翻转」提示；
  - **有用户图片**：展示用户编辑后的图片；可选底部中文 overlay 条（primary.cn）；「翻转」提示。
...
- **REQ-STUDY-16：** 详情抽屉应展示：分音节显示的单词、发音控制区（语速减/加、当前倍速、朗读按钮）、音标、**全部义项列表**（每项含词性 + 中文释义，primary 可标注）、各义项例句（或「暂无例句」）；词根词缀段落可空。不得仅展示一条拼合脏 `cn`。
...
- **REQ-QUIZ-1：** 默写题型为：展示 **primary** 中文释义（及 primary 词性、词头音标），用户输入对应英文单词。
- **REQ-QUIZ-2：** 每次进入测验页，系统须重新初始化：从服务端抽取题目（默认池见 REQ-TODAY-11；须满足 REQ-LEX-4），重置题号、得分与错题列表。
- **REQ-QUIZ-3：** 每题界面应包含：顶部进度条、题号与总题数、当前得分、题干卡片、「确认」按钮；默写含输入框，选择含选项列表。
- **REQ-QUIZ-4：** 进入默写题后，输入框应自动获得焦点。
- **REQ-QUIZ-11：** 系统应支持三种题型：`dictation`（默写）、`choice_en_cn`（英→中选择）、`choice_cn_en`（中→英选择）；默写与选择分别写入独立 skill 进度（见 api-modules §2.2）。选择题正确项与干扰项均使用各词 **primary** 的干净中/英文；答错后展示的正确答案须与正确项一致（REQ-LEX-2）。
- **REQ-QUIZ-12：** 用户可从单组、多组、全部已入组词或最近学习组发起测验；开测模式支持混合直开（`mixed`）或自由选题型/题数（`free_select`）。
- **REQ-QUIZ-13：** 无合格 primary（含 `reject`）的词不得出现在出题池；无法为选择题凑出合格互异选项时，该题应降级为默写或换词。

### 12.2 判题、掌握度与反馈

- **REQ-QUIZ-5：** 默写：点击确认或回车后，将输入与标准答案比较，**忽略大小写**；选择：提交所选选项 key。提交后控件应禁用防重复提交。
- **REQ-QUIZ-6：** 服务端判题后，在同一事务中写入 `review_events` 并更新该 `cardId + skill` 的 FSRS 状态：答错映射 `Again`，答对映射 `Good`；同时更新对应 `lexemeId + skill` 的词形熟悉度。
- **REQ-QUIZ-7：** 判题展示约 1.4 秒后，系统应自动进入下一题。
- **REQ-QUIZ-8：** 每个复习事件必须保存评分、题型、作答结果、旧/新 FSRS 状态与算法版本，支持完整审计。
...
| REQ-DATA-2 | 当前学习计划 | 创建或切换计划时 | 当前主词书、分组、今日任务、测验池 |
...
| REQ-DATA-8 | 卡片 FSRS + 词形熟悉度 | **有效测验判题后** | 分组、详情、今日任务与跨书诊断 |
| REQ-DATA-9 | FSRS 到期时间 | 测验判题后由服务端计算 | 今日页「到期复习」 |
...
> 产品文案可称「艾宾浩斯复习」；实现采用服务端权威 FSRS。

- **REQ-EBBING-1：** FSRS 使用锁定的算法版本、官方默认权重、目标记忆保持率 `0.90`、最大间隔 `36500` 天；首版不训练个人参数。
- **REQ-EBBING-2：** 有效答题答对映射 `Good`，答错映射 `Again`；客户端不得直接提交 FSRS 评分。
- **REQ-EBBING-3：** 今日待复习由当前计划中 `card_skill_memory.due_at <= now` 的卡片组成，并以服务端计算顺序为准。
- **REQ-EBBING-4：** 卡片翻转、浏览、发音和学习 session 上报不得写入双层记忆或 `review_events`。
- **REQ-EBBING-5：** 默写与选择分别维护独立 FSRS 状态；任一轨的作答不得覆盖另一轨。
- **REQ-EBBING-6：** `card_skill_memory` 是当前词书考义的唯一权威调度状态。
- **REQ-EBBING-7：** `lexeme_skill_memory` 仅用于新词书诊断；不得直接将新学习卡初始化为已掌握。
- **REQ-EBBING-8：** 已熟悉词切换到新词书后先进入诊断题，诊断结果用于初始化新 `card_skill_memory`。
- **REQ-EBBING-9：** 每次状态更新必须与 `review_events` 写入处于同一数据库事务；失败时两者同时回滚。
...
| 首页主入口 | 固定「开始学习」底栏 | 已实现固定底栏 | **最近学习组列表为主入口**（REQ-TODAY-1/7/13） |
...
| 2026-07-10 | v6.5 | 指针：词库结构化改造计划（plans/lexicon-restructure）；REQ 正文待 Phase A 修订 |
| 2026-07-09 | v6.4 | skill 双轨与题型；组测/最近组；热力展示与开测设置（REQ-QUIZ-11/12、TODAY-13、SETTINGS-8、EBBING-9） |
| 2026-07-10 | v6.6 | Phase A：REQ-LEX 字段职责与 1:n 义项；测验/卡片/详情/导入对齐 primary；REQ-QUIZ-13 |
